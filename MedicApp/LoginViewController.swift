...
class LoginViewController: UIViewController {
    ...
    func configureImagenPrivate(){
        imagenPrivate.translatesAutoresizingMaskIntoConstraints = false
        imagenPrivate.image = UIImage(named: "Privado")
        view.addSubview(imagenPrivate)
    }
    ...
    @objc func actionLogin(){
        if usuarioLabel.text == "A", passwordLabel.text == "1"{
            navigationController?.pushViewController(MenuOneViewController(), animated: true)
        } else {
    ...
    func configureLayout(){
        NSLayoutConstraint.activate ([
            ...
            imagenPrivate.bottomAnchor.constraint(equalTo: lineTextTwo.topAnchor, constant: -5),
            imagenPrivate.trailingAnchor.constraint(equalTo: view.trailingAnchor, constant: -60),
